...
When a dictation was transcribed correctly but never landed — focus moved, the
field wasn't a text field, the paste was refused — this pastes it again. It reads
the newest **Dictate** entry from the history, so a rewrite in between doesn't
become what you get.

Not Cmd+Shift+V: that's "paste without formatting" in Chrome, Slack, Notion and
most editors, and a Karabiner rule would shadow it everywhere.

If the keystroke can't be sent, the text is left on the clipboard with a
notification rather than being restored over — the shortcut still saves the
retyping even when it can't finish the job.
...
| `paste_last.py` | Cmd+Option+Shift+V: pastes the last dictation again |
