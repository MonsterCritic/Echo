...
### ↺ Paste the last dictation again — *no shortcut bound*
`paste_last.py` pastes the newest **Dictate** entry from the history again, for
when a dictation was transcribed correctly but never landed. It reads Dictate
entries specifically, so a rewrite in between doesn't become what you get, and if
the keystroke can't be sent it leaves the text on the clipboard with a
notification rather than restoring over it.

**It is deliberately not bound to a key.** Finding a free chord is the problem:
Cmd+Shift+V is "paste without formatting" in Chrome, Slack, Notion and most
editors, and Cmd+Option+Shift+V collides with Figma. A global Karabiner rule
shadows the app's own binding everywhere, which costs more than the shortcut
saves — and the menubar's **Recent** list already re-copies any of the last few
results in one click.

Bind it if you find a chord you don't use, by adding a rule to
`~/.config/karabiner/karabiner.json`:

```json
{
    "description": "Paste the last dictation again",
    "manipulators": [{
        "type": "basic",
        "from": { "key_code": "v", "modifiers": { "mandatory": ["command", "option", "control"] } },
        "to": [{ "shell_command": "/usr/bin/python3 ~/Documents/context-helper/paste_last.py &" }]
    }]
}
```
...
| `paste_last.py` | Pastes the last dictation again; no shortcut bound by default |
